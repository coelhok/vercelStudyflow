...
def create_signed_upload_url(storage_name: str) -> dict:
    """Cria URL assinada de upload para enviar arquivo direto do navegador ao Supabase Storage.

    Isso evita passar o binário pela Function da Vercel, contornando 413 FUNCTION_PAYLOAD_TOO_LARGE.
    """
    if not settings.supabase_url or not settings.supabase_service_key:
        raise RuntimeError("Supabase não configurado para upload direto.")
    storage_name = normalize_storage_path(storage_name)
    base_url = settings.supabase_url.rstrip("/")
    bucket = settings.supabase_bucket
    encoded_path = urllib.parse.quote(storage_name, safe="/")
    url = f"{base_url}/storage/v1/object/upload/sign/{bucket}/{encoded_path}"
    body = json.dumps({"upsert": True}).encode("utf-8")
    headers = {
        "Authorization": f"Bearer {settings.supabase_service_key}",
        "apikey": settings.supabase_service_key,
    ...
    try:
        ...
        _log(f"Signed upload URL falhou status={exc.code} body={raw[:500]}")
        raise RuntimeError(f"Falha ao criar URL assinada: HTTP {exc.code} {raw[:200]}") from exc

    signed_url = data.get("signedURL") or data.get("signedUrl") or data.get("url") or data.get("signed_url")
    token = data.get("token")

    # A API REST costuma retornar URL relativa começando com /object/upload/sign/...
    if signed_url and signed_url.startswith("/"):
        signed_url = f"{base_url}/storage/v1{signed_url}"
    elif signed_url and not signed_url.startswith("http"):
        signed_url = f"{base_url}/storage/v1/{signed_url.lstrip('/')}"

    if not signed_url:
        raise RuntimeError(f"Resposta sem signed URL: {data}")

    return {"storage_path": storage_name, "signed_url": signed_url, "token": token}


def download_from_supabase_storage(storage_path: str, target_path: Path) -> Path:
    """Baixa um arquivo privado do Supabase Storage usando service key."""
    if not settings.supabase_url or not settings.supabase_service_key:
        raise RuntimeError("Supabase não configurado para download.")
    storage_path = normalize_storage_path(storage_path)
    base_url = settings.supabase_url.rstrip("/")
    bucket = settings.supabase_bucket
    ...
    try:
        with urllib.request.urlopen(req, timeout=90) as response:
            ...
            _log(f"Download Storage ok path={storage_path} bytes={target_path.stat().st_size}")
            return target_path
    except urllib.error.HTTPError as exc:
        raw = exc.read().decode("utf-8", errors="ignore")
        _log(f"Download Storage falhou status={exc.code} body={raw[:500]}")
        raise RuntimeError(f"Falha ao baixar arquivo do Storage: HTTP {exc.code} {raw[:200]}") from exc
